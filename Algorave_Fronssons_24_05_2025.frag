...
buv = floor(buv/px)*px;
  ...
  vec3 col = vec3(0.);//texture2D(aliG, uva).xyz*MIDI_FADER(0);
  vec3 acc = vec3(0.);
  float depth = 1000.;
...
if (depth > 3.)
  ...
  col += 10.*texture2D(pixelDance, uv*sz+.5).xyz;
  //col *= .0;
//  col += texture2D(catEnergy, uva).xyz;
  ...
  return pow(sat(col.zxy), vec3(4.))*5.;
}
vec3 rdr2(vec2 uv)
{
  vec2 off = vec2(0.01, 0.)*(MIDI_KNOB(0.))*3.*FFT(abs(uv.y));
  vec3 col = vec3(0.);
...
uv *= r2d(MIDI_KNOB(3)*6.3);
// AHAHA Encore ce bon vieux lenny !
    float px = .1*sin(uv.x*10.+time*3.);
    uv = floor(uv/px)*px;
...
//uv = floor(uv/pix)*pix;
  ...
  float beat = 1./8.;
  col += vec3(.9,.4,.6)*sat(mod(time, beat)/beat)*MIDI_BTN_S(0.);
...
float sz = mix(1.,2., sat(sin(length(uv+.5)+time)));
  ...
  //col *= texture2D(aliG, buv+.5).xxx;
  //col *= FFT(abs(uv.x))+.5;;
col = sat(col);
col *= texture2D(circuitTex,buv+.5).xyz+.5;
col *= .5+ sat(sin(length(uv)*2.-time*3.));
  col = mix(col, (texture2D(backbuffer, (ouv-.5)*.8*r2d(MIDI_FADER(2)-.5)+.5).xyz).zxy, MIDI_KNOB(2));
  vec3 cookie = pow(texture2D(cookieTex, suv+.5).xyz, vec3(2.));
  col.xy *= r2d(-time*5.+lenny(suv)*10.+atan(suv.y, suv.x));
col.zy *= r2d(time);
//col = abs(col);
  //col = mix(col, cookie, .5);
  col = col.xyy;
