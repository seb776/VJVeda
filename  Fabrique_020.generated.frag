...
vec3 getMat(vec3 p, vec3 n, vec3 rd, vec3 res)
{
  ...
  if (res.z == 2.)
  {
    ...
    col = vec3(.8,.2,.9)*(1.-sat(min(grid.x,grid.y)*100.));
  }
  if (res.z == 3.)
    col = vec3(0.);
  return col;
...
col *= pow(FFT(uv.x*.5),2.)*15.;
col = mix(col, texture2D(backbuffer, ouv).xyz, .8);

  float repbar = .2;
  float id = mod(buv.x+repbar*.5,repbar)-repbar*.5;
  float h = FFT(abs(id)*.05)*.1;
  col *= 1.-sat((abs(uv.y)-h)*4.);
  col.xy *= r2d(FFT(uv.y*.1)*10.);
  col = abs(col);
