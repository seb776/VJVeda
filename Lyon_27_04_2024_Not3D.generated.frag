...
float gnd = -p.y-textureRepeat(circuitTex, p.xz*.1).x*3.;
...
vec3 rdr(vec2 uv)
{
  ...
  for (float i = 0.; i < 128.; ++i)
  {
    ...
    if (d < 0.01) {
      ...
      col = (vec3(sin(p.z))+rgb)*sat(i/64.);
      depth = distance(p, ro);
      break;
    }
    acc += vec3(1.)*sat(d/.5)*.0001;
    p+=rd*d;
  }
...
float sz = mix(4.,.5, hash11(id*.1+.5));

uv.x = mod(uv.x+rep*.5,rep)-rep*.5;
if (depth > 3.)
  col += texture2D(pixelDance, uv*sz+.5).xyz*.3;
...
float sz = mix(1.,2., sat(sin(length(uv+.5)+time)));
  col *= textureRepeat(circuitTex, (uv*sz)+.5).xxx+.7;

  col = sat(col);
  col = mix(col, texture2D(backbuffer, ouv).xyz, .1);
